...
post_body = {
    ...
    "chrome_web_icon": app.config["PROTOCOL"]
    + app.config["SERVER_NAME"]
    + "/static/android-chrome-512x512.png",
    "chrome_web_badge": app.config["PROTOCOL"]
    + app.config["SERVER_NAME"]
    + "/static/android-chrome-512x512.png",
...
def send_notification(
    notification_body: dict, subscriptions: list, client: onesignal.Client
) -> None:
    """
    ...
    tokens: list[str] = list(
        map(lambda subscription: subscription.token, subscriptions)
    )
    ...
    if onesignal_response.status_code == 200:
        app.logger.info(
            "The notification ({}) sent out successfully".format(notification.post_body)
        )
    else:
        app.logger.warn(
            "The notification ({}) was unsuccessful".format(notification.post_body)
        )
...
def packet_signed_notification(packet: Packet, signer: str) -> None:
    ...
    notification_body["chrome_web_icon"] = (
        "https://profiles.csh.rit.edu/image/" + signer
    )
...
def packet_100_percent_notification(packet: Packet) -> None:
    ...
    member_subscriptions = NotificationSubscription.query.filter(
        cast(Any, NotificationSubscription.member).isnot(None)
    )
    ...
    if member_subscriptions or intro_subscriptions:
        notification_body = post_body
        notification_body["contents"]["en"] = (
            packet.freshman.name + " got 💯 on packet!"
        )
        ...
        send_notification(
            notification_body, intro_subscriptions, intro_onesignal_client
        )
...
def packet_starting_notification(packet: Packet) -> None:
    ...
    notification_body = post_body
    notification_body["contents"]["en"] = (
        "Log into your packet, and get started meeting people!"
    )
...
def packets_starting_notification(start_date: datetime) -> None:
    ...
    notification_body = post_body
    notification_body["contents"]["en"] = (
        "New packets have started, visit packet to see them!"
    )
