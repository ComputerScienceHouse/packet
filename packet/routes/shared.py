...
def logout() -> Response:
    """
    Log out the user and redirect to the CSH homepage.
...
def freshman_packet(
    ...
    Returns:
        Union[str, Tuple[str, int]]: The packet information or an error message.
    """
...
def packet_sort_key(packet: Packet) -> Tuple[str, int, bool]:
    ...
    Args:
        packet (Packet): The packet to generate the key for.

    Returns:
        Tuple[str, int, bool]: The sorting key for the packet.
    """
...
def packets(info: Dict[str, Any]) -> str:
    ...
    Returns:
        str: The rendered template for the active packets page.
    """
...
def update_service_worker() -> Response:
    """
    Serve the update service worker for push notifications.
...
def not_found(e: Exception, info: Optional[Dict[str, Any]] = None) -> Tuple[str, int]:
    ...
    Returns:
        Tuple[str, int]: The rendered template and status code.
    """
