...
from .ldap import ldap_get_eboard, ldap_get_live_onfloor, ldap_get_active_members
...
def create_packets(freshmen_csv):
    ...
    offfloor = set(member.uid for member in set(ldap_get_active_members()) - set(ldap_get_live_onfloor()))
    all_upper = eboard.union(onfloor).union(offfloor)
...
def ldap_sync():
    ...
    for packet in Packet.query.filter(Packet.end > datetime.now()).all():
        ...
        # Migrate UpperSignatures that are from accounts that are not eboard or active anymore
        ...
        # Migrate MiscSignatures that are from accounts that are now eboard or active members
        ...
        # Create UpperSignatures for any new eboard or active members
        # pylint: disable=cell-var-from-loop
