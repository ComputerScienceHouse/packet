    ...
    try:
        with open(freshmen_csv, newline="") as freshmen_csv_file:
            return {
                ...
                for freshman in map(CSVFreshman, csv.reader(freshmen_csv_file))
            }
...
def fetch_results(file_path: str, use_csv: bool, date_str: str) -> None:
    ...
    try:
        end_date = datetime.combine(
            datetime.strptime(date_str, "%m/%d/%Y").date(), packet_end_time
        )
    except ValueError:
        end_date = datetime.combine(
            input_date(
                "Enter the last day of the packet season you'd like to retrieve results "
                "from"
            ),
            packet_end_time,
        )
...
def extend_packet(packet_id: int) -> None:
    ...
    print(
        "Ready to extend packet #{} for {}".format(packet_id, packet.freshman_username)
    )

    packet.end = datetime.combine(
        input_date("Enter the new end date for this packet"), packet_end_time
    )
...
def remove_sig(packet_id: int, username: str, is_member: bool) -> None:
    ...
    if is_member:
        sig = UpperSignature.query.filter_by(
        ...
        sig = FreshSignature.query.filter_by(
            packet_id=packet_id, freshman_username=username
        ).first()
